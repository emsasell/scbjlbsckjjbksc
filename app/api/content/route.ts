...
function payload(x:any){
 ...
 let published_at=safePublished(x.published_at,kind);
 const requested=cleanText(x.status);
 let status=(kind==='news'||kind==='video')?(published_at&&published_at.getTime()>Date.now()?'scheduled':'published'):'published';
 if(kind==='news'||kind==='video'){
   if(requested==='draft') status='draft';
   else if(requested==='scheduled'){ if(!published_at||published_at.getTime()<=Date.now()) throw new Error('Для отложенной публикации укажите будущее время.'); status='scheduled'; }
   else if(requested==='published'){ status='published'; published_at=new Date(); }
 }
