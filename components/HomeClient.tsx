...
function TabPostsView({posts}:{posts:any[]}){if(!posts.length)return null;return <section className="tab-materials"><div className="tab-materials-head"><span className="eyebrow">МАТЕРИАЛЫ ВКЛАДКИ</span><h3>Новости и материалы</h3></div><div className="tab-materials-list">{posts.map((post:any,i:number)=><article className="tab-material" key={i}>{post.image_url&&<img src={mediaSrc(post.image_url)} alt="" onError={e=>{const img=e.currentTarget as HTMLImageElement;img.alt='Не удалось загрузить изображение';img.classList.add('media-error')}}/>}<div><span>Материал {i+1}</span>{post.title&&<h4>{post.title}</h4>}<p>{post.body}</p></div></article>)}</div></section>}
