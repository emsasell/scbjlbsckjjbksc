...
export default function HomeClient({content:initialContent}:Props) {
 ...
 return <div className="site-shell">
  ...
  <main>
   {active==='Главная' && <>
    <section className="hero"><div className="hero-copy"><div className="eyebrow">MINECRAFT BEDROCK • MEGAMINE UTC+4</div><h1>Добро пожаловать<br/><em>в MegaMine</em></h1><p>Живой мир, события, районы и новости проекта — всё в одном месте.</p><div className="hero-actions"><button onClick={()=>setActive('Новости')}>Смотреть новости</button><a href="#about">О проекте ↓</a></div></div><div className="hero-avatar"><img src="/avatar.jpg" alt="MegaMine"/><div className="pixel-card">MEGAMINE<br/><small>ТОЛЬКО BEDROCK</small></div></div></section>
    <section className="stats"><div><b>{content.news.length}</b><span>новостей</span></div><div><b>{content.districts.length}</b><span>районов</span></div><div><b>{content.links.length}</b><span>каналов</span></div><div><b>BEDROCK</b><span>только Bedrock</span></div></section>
...
function InfoModal({item,onClose}:{item:Item;onClose:()=>void}){
 ...
 return <div className="modal-backdrop" onMouseDown={onClose}><article className="info-modal" onMouseDown={e=>e.stopPropagation()} role="dialog" aria-modal="true" aria-label={item.title}>
  ...
  <div className="modal-content"><span className="eyebrow">{item.kind==='district'?'РАЙОН MEGAMINE':'НОВОСТЬ MEGAMINE'}</span><h2>{item.title}</h2>{item.published_at&&<time>{megaTime(item.published_at)} MegaMine</time>}<p>{item.body}</p>
